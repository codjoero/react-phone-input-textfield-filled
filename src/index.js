...
class PhoneInput extends React.Component {
  ...
  constructor(props) {
    ...
    const onlyCountries = this.excludeCountries(
      this.getFilteredCountryList(
        props.onlyCountries,
        filteredCountries,
        props.preserveOrder.includes("onlyCountries")
      ),
  ...
  getFilteredCountryList = (countryCodes, sourceCountryList, preserveOrder) => {
    if (countryCodes.length === 0)
      return this.extendCountries(sourceCountryList);
    ...
    if (Object.keys(areaCodes).length > 0) {
      ...
      for (let i = 0; i < countries.length; i++) {
        ...
        if (areaCodes[countries[i].iso2] !== undefined) {
          ...
          if (countries[i + 1] && countries[i + 1].iso2 === foundCountry.iso2)
            continue;
          this.getCustomAreas(
            foundCountry,
            areaCodes[countries[i].iso2]
          ).forEach(o => {
            updCountries.push(o);
          });
          foundCountry = null;
  ...
  getSearchFilteredCountries = () => {
    ...
    if (this.props.disableAreaCodes)
      searchedCountries = this.deleteAreaCodes(searchedCountries);
    ...
    let countryDropdownList = searchedCountries.map((country, index) => {
      const itemClasses = classNames({
        ...
        highlight: countryIsPreferred
          ...
          : highlightCountryIndex === index - preferredCountries.length
      });
